...
$markerPath = Join-Path $HOME '.ixion' '.seeded'
...
if ($needsSeed) {
    ...
    # Start IXION in background
    Write-Output "Starting IXION (background)..."
    $serverProc = Start-Process -FilePath 'C:\Python314\Scripts\ixion-web.exe' -PassThru -WindowStyle Hidden

    # Wait for health endpoint
    $seedUrl = if ($env:IXION_SEED_URL) { $env:IXION_SEED_URL } else { "http://127.0.0.1:8000" }
    $healthUrl = "$seedUrl/api/health"
    Write-Output "Waiting for IXION at $healthUrl ..."
    $timeout = 60
    $elapsed = 0
    ...
    if ($elapsed -ge $timeout) {
        Write-Output "ERROR: IXION did not start within ${timeout}s. Skipping seed."
    } else {
        # Run seed_all.py
        Write-Output ""
        Write-Output "Running seed_all.py ..."
        # Set IXION_DATA_DIR to home dir for local dev (marker goes to ~/.ixion/.seeded)
        [Environment]::SetEnvironmentVariable('IXION_DATA_DIR', $HOME, 'Process')
    ...
    Write-Output "Restarting IXION in foreground..."
