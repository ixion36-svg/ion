...
**~~Residual question~~ — CLOSED v0.31.15.** New
`ION_AI_CHAT_RETENTION_DAYS` env var (default unset = disabled,
same opt-IN pattern as G2/G3). When set to positive integer N,
`data_retention_service.py` deletes `ai_chat_messages` rows whose
`created_at` is older than N days. Note: targets MESSAGES, not
sessions — sessions remain (the UI handles empty sessions; user-
controlled deletion of a session is still the way to remove the
whole conversation). The audit-doc abstraction paid off — closure
was one tuple appended to `RETENTION_RULES`, no new module / lock /
startup wiring.
