...
**Assessment Date:** 2026-09-23 (current: v0.99.2). Per-version deltas and history: see CHANGELOG + git history.
**Application Version:** 0.99.2 — **Net-new surfaces:** none — a documentation correction with no functional change. **A finding recorded at v0.99.1 is withdrawn.** That release reported that sign-in took materially longer for an existing account than for an absent one and carried an open Low on that basis, moving this document off zero for the first time since v0.34.0. The measurement was wrong: `/api/auth/login` is rate limited, the sample crossed that limit partway through, and throttled responses (which return immediately) were compared against genuine ones. Measured correctly — under the limit, asserting the status code of every response, and again at the service layer over fifteen samples per case — the two paths differ by **under one millisecond**, because sign-in already verifies the supplied password against a fixed dummy hash when the account does not exist. The defence was present and working the whole time; the open-findings table returns to **0 Critical / 0 High / 0 Medium / 0 Low**. The v0.99.1 note about the response-timing header is corrected in place: withholding it outside development remains sensible, since a deployment has no reason to publish server-side timing, but it is not an account-enumeration control and no longer claims to be. Two tests now pin the behaviour that was wrongly reported as absent — that the absent-account path still performs the hash comparison, and that the dummy hash costs the same as a real one. **Net new findings: 0C / 0H / 0M / 0L.** Prior version: 0.99.1 — **Net-new surfaces (ITHC remediation):** none — no new route, permission, schema, external sink, or dependency; one vendored library is updated and one user preference is removed. This release answers an external IT Health Check raised against v0.88, and every item below was re-verified against a running container before and after. **Closed:** the HTML sanitiser failed open, returning unsanitised markup into the DOM across sixteen call sites whenever DOMPurify failed to load, and its default configuration permitted `id`/`name` (a DOM-clobbering primitive the reported payload used); all sanitiser calls now share one fail-closed policy and DOMPurify moves 3.2.4 → 3.4.15, closing six advisories affecting 3.2.4. A malformed request is answered by the ASGI server before the application runs, so the header middleware never executed and the server banner leaked on that path alone — suppressed at every entry point, confirmed by A/B against the 0.99.0 image. Authorisation failures named the required permission, handing a caller the permission taxonomy; the caller now receives only a refusal and the permission reaches the audit log, matching the v0.88 sign-in contract. The AI assistant's prompts granted capability and constrained nothing: it now refuses role-change framing, declines ready-to-run offensive tooling regardless of stated justification (while continuing to analyse samples, explain techniques and author detection content), refuses discriminatory output, and states that it cannot read runtime state rather than inventing it. **Findings closed that the ITHC did not raise:** a per-user "custom instructions" preference appended caller-controlled text *after* those conduct rules and could override them — removed end to end; and the chat model identifier was a free-form field passed to the inference service, so a caller could select a model that never received the rules — removed. Uploaded files are now hashed, inspected for recognisable offensive patterns, audit-logged and passed to the model labelled as hostile data; uploads are deliberately **not** refused, since handling live samples is the product's purpose. Chat requests are bounded in message count, per-message and total size, sampling temperature and reply length. **Correction to the ITHC report:** the assistant's apparent disclosure of internal platform data was **fabrication, not disclosure** — there is no tool-calling in the chat path, the system prompt carries no runtime telemetry, and the reported version string matches nothing in this product; the defect is genuine (a security tool inventing system state can misdirect an investigation) but no data left the system. Quill 2.0.3 was reported outdated; it is the current release, and the low-severity advisory affecting it has no fixed version available. A response header publishing server-side request timing is now limited to development mode; it is unnecessary detail in a deployment, though it does **not** leak account existence — sign-in verifies against a dummy hash when the account is absent, and the two paths measure equal. **Net new findings: 0C / 0H / 0M / 0L.** Prior version: 0.99.0 — **Net-new surfaces (quality + CI pass):** none — no new route, permission, schema, external sink, or dependency; this release removes code and closes a gap. **Assurance defect closed:** the AIFeedback ledger's close-side write fired on only one of five case-close paths, so the record of how Bob's verdicts compared to human outcomes was incomplete on four of them — a completeness defect in an audit trail, not an access-control one; all five paths now route through one helper that cannot fail the close, and historical gaps are not backfilled. Integration fan-outs that previously issued up to ~160 sequential requests in one call now run concurrently under an explicit cap, so the fix cannot itself become a connection storm against a single-node OpenCTI or DFIR-IRIS. **Supply chain:** the Ollama image is pinned to a version instead of tracking `latest`, which an air-gapped deploy cannot re-pull identically, and CI now builds the runtime image and asserts its SBOM is still produced — previously nothing built the Dockerfile before a release tag was cut. The test suite additionally runs on the Python version the image actually ships, which had never been exercised. The v0.98.0 `style-src` hash workaround is gone and the strict nonce policy (**no `unsafe-inline`**, no hash) is unchanged. **Net new findings: 0C / 0H / 0M / 0L.** Prior version: 0.98.0 — **Net-new surfaces (Flow Topology phase 2):** none of consequence — no new route, permission, schema, or dependency. The topology payload's edges gain two derived fields: a threat overlay computed from the same observable-ledger context the nodes already carried, and a representative `community_id` from the Arkime session sample. That value originates in capture data, so it is **charset-gated at capture** (`[0-9]:[A-Za-z0-9+/=]{8,96}`) before it may be interpolated into the quoted Arkime deep-link expression, and the frontend keeps the existing `^https?://` + escape gate on every href. The graph renderer moved from vis-network to the already-vendored Cytoscape; its single injected stylesheet (`position: relative` on its own container) is suppressed at source — the page pre-declares that rule in its own nonce'd `<style>` under the id Cytoscape checks for, so the injection never occurs and **the CSP is unchanged** (strict nonce policy, **no `unsafe-inline`**, no hash to maintain). Cytoscape renders labels as plain canvas text (not HTML), so capture-derived strings gain no new injection sink; the detail panel keeps its own escaping. Separately, the v0.87.0 GitLab tuning-request mirror is now **verified end-to-end** against a purpose-built mock (`test-gitlab/mock_gitlab.py`, a dev harness that is not part of the shipped application) and pinned in CI, including the mirror-failure-never-blocks contract. **Net new findings: 0C / 0H / 0M / 0L.** Prior version: 0.97.0 — **Net-new surfaces (DE module licensing + content packs + PCAP note evidence):** the Detection Engineering module gains an optional licence gate — an offline Ed25519-signed JWT verified against a public key baked into the image (`ion.licensing`), a new trust input but one that only ever *restricts* access (a missing/invalid licence makes the module dark, never grants anything); enforcement ships **off by default**, so the module's exposure is unchanged until an operator turns it on. Two new authenticated routes, `GET /api/de/content-pack/export` (`de:read`) and `POST /api/de/content-pack/import` (`de:propose`), move curated quirks + proposals between deployments; **import cannot bypass governance** — it routes through the existing raise/create paths so imported quirks land `pending` with no verifier (separation-of-duties intact), proposals land `draft` with no decision, wildcard scopes are rejected, and re-import dedups. No new external sink or dependency (PyJWT/cryptography already present). Separately, PCAP auto-analysis case notes now render the extracted protocol evidence behind each verdict (IKE/ISAKMP sessions incl. auth-failure notifications, and a credentials-**observed** section that records protocol/username/endpoints but **never writes the captured secret** into the note). Multi-tenancy hardening removed dead phase-2 scaffolding and gave the tenant-switch cookie the `Secure`-on-HTTPS behaviour the session cookie already had. **Net new findings: 0C / 0H / 0M / 0L.** Prior version: 0.96.3 — **Net-new surfaces:** none — a cosmetic patch. The Verdict Review page no longer probes `/api/response/actions` when SOAR response actions are disabled (the endpoint deliberately 404s when off); a server-rendered flag now gates the fetch, removing a console 404. No route, permission, schema, sink, or dependency change, and behaviour is identical when response actions are enabled. **Net new findings: 0C / 0H / 0M / 0L.** Prior version: 0.96.2 — **Net-new surfaces:** none — a correctness patch. Every case-close/transition path now mirrors the case status onto its linked Kibana case inline (new shared helper `push_case_status_to_kibana`), instead of only the manual PATCH path doing so while the rest waited on the best-effort periodic reconciler. No new route, permission, schema, sink or dependency; the change only makes an existing ION→Kibana status write happen on more of the paths that already close cases. **Net new findings: 0C / 0H / 0M / 0L.** Prior version: 0.96.1 — **Net-new surfaces:** none — a review-fix patch on phase-1 tenancy. Material corrections to the v0.96.0 posture text below: tenant binding was set in a copied context and never reached the request (every request read the default estate; now bound in the request's own context); an unresolved tenant now fails **closed** — a user bound to a deactivated estate, or a resolution error, is refused (403/503) rather than falling through to the default estate's cluster, so "binding never raises" no longer holds, by design; platform-global now requires NULL `tenant_id` **plus the admin role**, so pre-existing (all-NULL) analysts cannot reach a new client estate; Kibana Cases and one analytics ES cache are tenant-routed (first-caller config was pinned for the process lifetime); a tenant's basic auth no longer loses to the inherited process `api_key`, and the kibana←ES credential fallback stays inside the tenant. **Net new findings: 0C / 0H / 0M / 0L.** Prior version: 0.96.0 **Net-new surfaces (multi-tenancy, phase 1):** one new table (`tenants`: slug, name, active, default — identity only, no credentials), one nullable column (`users.tenant_id`), and two authenticated routes (`GET /api/tenants`, `POST /api/tenants/switch`). All inert unless `ION_MULTI_TENANT` is set. **Connection credentials deliberately do not live in the database** — a tenant's Elasticsearch and Kibana settings come from `ION_TENANT_<SLUG>_*` in `.env`, so this adds no new secret store; it is the existing configuration surface made per-estate. Those credentials are a cluster service account, so alert isolation is enforced by that account's Elastic role and Kibana space rather than by ION applying a filter. **The switcher cannot widen access:** the selection travels in a caller-controlled cookie that is re-validated on every request by the same resolver, which refuses a tenant-bound user any estate but their own and logs the attempt; a forged value therefore grants nothing a legitimate switch would not. An unresolved tenant binds to none rather than to all — every scoped query must read that as no rows, so a failed lookup hides data instead of exposing another estate's. Tenant binding never raises, so a tenancy fault cannot break authentication. Tenant names reach the DOM and are escaped. **Known and deliberate:** ION's own rows (cases, triage, notes, observables, case embeddings) are **not** tenant-scoped in this phase and are shared across estates; ION's RBAC remains global, so a role grants the same permissions in every estate a user can reach. Both are phase-2 work and are called out because vector similarity returns its k nearest regardless of filtering, which makes an unscoped case-embedding query look like a working feature. **Findings closed:** a process-wide cached Elasticsearch service would have served the first tenant's cluster to every later caller; the client pool would have leaked a superseded client on credential rotation. **Net new findings: 0C / 0H / 0M / 0L.** Prior version: 0.95.0 **Net-new surfaces (chat retrieval + flag defaults):** no new route, permission, schema, external sink or dependency. The material change is that four flags which shipped **off** now ship **on**, so two previously-inert write paths become active by default: per-user pinned alert fields and Bob's human-reviewed investigation templates, both assessed at v0.91.0 (user-scoped, ownership enforced in-handler, `prompt_safety`-fenced, `pending_review` before trust) — unchanged, simply no longer dormant. Chat retrieval now also reads the KB-chunk and playbook embeddings the background loops already maintain; it embeds the analyst's own question, writes nothing, and filters on the querying model tag so vectors are never compared across embedding regimes. `ION_CHAT_GROUNDING_CHECK` adds one extra temp-0 call to the same on-prem Ollama per grounded answer — no new egress — and is advisory only: it runs after the answer has streamed and cannot withhold or rewrite it. **Findings closed:** exception text and remote-controlled GitLab assignee names reached `innerHTML` unescaped across 18 sites in three templates while every sibling interpolation escaped; code scanning flagged 2 of them. Chat sampled at 0.7 against 0.1–0.4 everywhere else, which is a fabrication risk rather than a vulnerability, and the reference block now states an explicit grounding contract. **Unchanged and noted:** the nine internal integrations still default to `verify_ssl=false` for self-signed air-gapped endpoints and `ION_CA_BUNDLE` remains the fix — but the startup validator now names every *enabled* integration running unverified and how to turn it on, where previously only OIDC warned and only lazily. Account lockout and PII anonymisation remain opt-in by decision. **Net new findings: 0C / 0H / 0M / 0L.** Prior version: 0.94.0 **Net-new surfaces (security defaults, ASGI middleware, deployment tunables):** no new route, permission, schema or external sink. This release *removes* surface and *closes* defaults. Three controls that shipped disabled now ship enabled, and two fail closed rather than warn: a weak or unset `ION_ADMIN_PASSWORD` aborts startup (the unset value fell through to the literal `changeme`, and `admin` is the only local account), and `ION_DB_PASSWORD` is mandatory — `POSTGRES_PASSWORD` and the connection string in both the `ion` and `seeder` services previously defaulted to a password published in this repository. Inbound webhooks now require an HMAC signature by default, and a secret-less webhook is refused at creation rather than issued a token that can never deliver. `ION_DEV_MODE` is the single, logged opt-out; it relaxes only the weak-credential check and the Secure cookie flag, and announces itself at startup. **Findings closed:** webhook create/update/delete/regenerate-token were guarded by a dependency accepting `alert:read`, which six of nine seeded roles hold without `integration:manage` — an analyst could rotate webhook credentials; the four mutations now require `integration:manage` while the read endpoints keep the permissive dependency by design. A 465-line SIEM exporter carrying an unvalidated outbound HTTP sink is deleted: it was unreachable (nothing imported it, and the SIEM export actually served makes no outbound request), and an unguarded sink that no one calls is what gets wired up later without a guard. Eleven silently swallowed exceptions in the alert UI now log, which also surfaced that `X-Trace-ID` was discarded whenever `traceparent` was absent, breaking trace correlation. **Unchanged and noted:** eight integrations still default to `verify_ssl=false` for self-signed air-gapped endpoints; `ION_CA_BUNDLE` remains the supported fix and all eight flags are now visible in `.env.deploy` rather than discoverable only from source. **New dependency:** `uvicorn[standard]` adds uvloop and httptools, two C extensions from the existing uvicorn project. **Net new findings: 0C / 0H / 0M / 0L.** Prior version: 0.93.2 — **Net-new surfaces (Tailwind + daisyUI styling changeover):** none. v0.93.0–v0.93.2 replace the hand-written stylesheet layer with a compiled Tailwind + daisyUI build and fix the regressions that changeover exposed. No new route, permission, schema, external sink, or dependency; no Python behaviour changes. The build is air-gap-safe — daisyUI is vendored and compiled by a committed standalone binary with no Node toolchain, no CDN reference, and no network fetch at build or run time, and the Tailwind version is pinned (`frontend/TAILWIND_VERSION`) and guarded by a test so the shipped stylesheet cannot silently be produced by a different compiler. Content-scan globs are explicit (`source(none)` plus four paths), which removed ~90% of the previously generated utility layer — class names swept out of the changelog, docs and tooling data. CSP is unaffected: runtime-computed styles continue to travel in a `data-ion-style` attribute applied via `setProperty`, never an inline `style=` attribute. **Net new findings: 0C / 0H / 0M / 0L.** Prior version: 0.92.0 — **Net-new surfaces (CSRF protection):** none. This release adds a *control*, not a capability: no new route, permission, schema, external sink, or dependency. A new `CSRFMiddleware` enforces two independent checks on every cookie-authenticated state-changing request — a per-session HMAC token (`X-CSRF-Token`, derived from the session token, so no new secret is stored or distributed) and Origin/Referer validation. Enforcement is **ON by default** (`ION_CSRF_ENABLED`), deliberately breaking from ION's opt-in flag convention because this restricts behaviour rather than adding it. A `Bearer` request carrying no session cookie is exempt from the token check — CSRF exploits credentials the browser attaches automatically and a Bearer token is not one of those — but Origin validation still applies to it, so the control also covers Bearer-authenticated browser traffic. Two routes are exempt by necessity and audited as such: `POST /api/auth/login` (no session exists yet to bind a token to; already rate limited) and `POST /api/integrations/webhooks/receive/{token}` (inbound machine traffic, authenticated by the path token). **This closes a documented gap rather than a live hole:** the session cookie was already `HttpOnly` + `SameSite=strict`, so current browsers do not attach it cross-site and the headline attack was already blocked; this is defence in depth against a future `SameSite` relaxation, same-site attacker positions, and older or buggy clients. **Net new findings: 0C / 0H / 0M / 0L** — the CSRF row in the control table moves from *OIDC state parameter + SameSite cookies* to full token + Origin enforcement. Prior version: 0.91.0 — **Net-new surfaces (Alert-detail redesign + Bob custom investigation templates):** this release is UI-forward and ships behind four opt-in flags (`ION_ALERT_DETAIL_V2`, `ION_ALERT_FIELD_PINS`, `ION_BOB_CUSTOM_TEMPLATES`), all default **OFF** and inert until enabled. Two new authenticated, `alert:read`-gated write paths appear. **Pinned fields** (`/api/alerts/pinned-fields`): each analyst pins alert field *names* to their own Case-context view, persisted per-user, scoped per rule with a global fallback; every query is scoped to the caller's own `user_id` (no cross-user read or write, ownership enforced in-handler before the mutation), and the stored value is an ECS field name, not adversary-controlled content. **Bob custom investigation templates** (`/api/alerts/investigation-template/...`): an on-demand, human-reviewed generation of an investigation checklist for a rule — it reuses the existing auto-investigation evidence path and `prompt_safety` (all rule/alert text sanitised and fenced as untrusted before the model), writes a `pending_review` row a human must approve before it is trusted, and adds no new external sink or dependency. Two new first-party tables (`analyst_pinned_fields`, `rule_investigation_templates`); no change to existing schemas. **Net new findings: 0C / 0H / 0M / 0L** — opt-in, permission-gated, user-scoped, human-in-the-loop, injection-fenced. Prior version: 0.90.0 — **Net-new surfaces (Response Actions + Verdict Review):** this release adds ION's first *active-response* capability, and it is gated accordingly. **Response Actions** — a previously-dormant on-prem action engine (block / isolate / disable, via the existing integration adapters) is exposed behind a master switch (`ION_RESPONSE_ACTIONS_ENABLED`, default **OFF**); the surface 404s entirely when disabled. When on, every action is human-in-the-loop (no auto-execution), a high-risk action requires a *second* human to approve (separation of duty, new `response:approve` permission), and execution is forced to a dry-run unless a second explicit switch (`ION_RESPONSE_ACTIONS_LIVE`, default **OFF**) is also set — so a stock or cautiously-enabled deployment cannot dispatch a live containment action. Every request / approve / reject / execute transition is audit-logged and mirrored to the case as a Kibana-synced Note. **Verdict Review** (new `verdict:review` permission) is a read/resolve surface over Bob's own suggested verdicts (first-party AI-feedback rows); resolving stamps the existing ledger. The two connect: a reviewer may propose a containment action from a verdict, which enters the same approval queue. Targets are operator/analyst-selected observable values validated at the adapter; no adversary-controlled content reaches a new sink, and there is no new external dependency or schema. **Net new findings: 0C / 0H / 0M / 0L** — the active-response capability ships default-off, permission-gated, human-in-the-loop, separation-of-duty, dry-run-first, and fully audited. Prior version: 0.89.0 — **Net-new surfaces (case-description refinement):** none of security consequence. The case description is now leaner (linked-alert IDs and the observable dump are no longer rendered — alerts show as first-class case links; observables move to a case Note), and a case's enrichable observables are listed in the existing first-party "Observables" case Note (mirrored to Kibana) even when unenriched, gaining OpenCTI context as it lands. The note is first-party observable data the caller can already see, rendered through the existing note→Kibana escaping path as a bulleted list (no GFM table — EUI-safe); observable values are flattened so an adversary-controlled value cannot break the note layout. No new route, permission, schema, external sink, or dependency. **Net new findings: 0C / 0H / 0M / 0L.** Prior version: 0.88.0 — **Net-new surfaces (Flow Topology + security-monitoring release):** (1) Flow Topology (NSE view) — a read-only visualisation joining the existing Arkime conversation graph to ION's first-party asset/threat context; no new external sink and no new write path. (2) Audit-subsystem tightening — the inbound request attack-detector now suppresses its payload-pattern checks on authenticated traffic (a SOC analyst legitimately handles malicious content, so those matches were almost pure false-positive; `ION_SECURITY_SCAN_AUTHENTICATED` restores the prior behaviour), while structural checks that signal an attack on ION itself continue to run for all callers. It also gains authorization-failure auditing: 401/403 responses are recorded, attributed to the acting user/IP, and escalate to a HIGH `security_event` on a per-actor burst (`ION_AUTHZ_ALERT_*`). This is detection surface only — no new inbound route, no new authenticated capability. (3) A proactive hardening pass closed several latent defence-in-depth items — stricter validation on an optional integration's query path, access-control alignment on the internal assessment module so its data endpoints match their siblings' permission gate, and uniform login responses that remove a user/account-state enumeration oracle (the specific reason is retained in the audit log, never returned to the caller) — plus hardened password defaults in the shipped deploy template. No new dependency or schema. **Net new findings: 0C / 0H / 0M / 0L** (the hardening items are proactive closures, not counted regressions). Prior version: 0.87.0 — **Net-new surfaces (DE Workbench release):** (1) `/api/de/tuning-requests` family — create is deliberately `alert:read` (any analyst may raise a request; it writes only ION's own `tuning_requests` row), queue actions (triage/link-proposal/close) are `de:propose`, listing `de:read`; every transition audit-logged. (2) New page `/de` (DE Workbench), `de:read`, read-composition of existing DE endpoints. (3) New OUTBOUND writes to GitLab via the existing integration: issue create/comment/close mirroring a request's lifecycle — best-effort (failure never blocks the analyst flow), no ION data beyond the request's own fields and rule-level FP counts leaves; nothing is read back. No new dependency, no new auth mode, no new table beyond `tuning_requests`. Prior version: 0.86.0 — net-new: Arkime hunts (lead-gated), WISE IOC feed (token-gated, off by default), streaming case-PCAP export, default-off tag write-back; plus the Keycloak/API-key auth-surface removal. Prior per-version net-new-surface notes: see CHANGELOG + git history.
...
| Severity | Open findings (v0.99.2) |
|----------|:-----------------------:|
| Critical | 0 |
| High | 0 |
| Medium | 0 |
