...
**Assessment Date:** 2026-08-13 (current: v0.81.2). Per-version deltas and history: see CHANGELOG + git history.
**Application Version:** 0.81.2 — **A read-only presentation change on data already served; no new endpoint, permission, schema or data flow.** The KEV catalogue page renders the description, required action, CWEs, references and provenance that `/api/kev` already returned and the row renderer discarded; `cwes` and `synced_at` are added to the serialiser. The catalogue is public reference data that ships inside the image, and nothing fetches cisa.gov at runtime — enforced by an existing test. No permission changes: the page is behind the same gate it already was. **Correctness-relevant**: ransomware status is published by CISA as Known/Unknown, never No, and the page previously showed a badge only for Known, so 1,324 of 1,662 entries rendered identically to a definitive negative — asserting an absence of risk the catalogue does not state. Unknown is now labelled as not-established. Catalogue values are external data reaching the DOM via innerHTML and are escaped, covered by test; references render as inert text rather than anchors, which also removes any navigation target from third-party content. CWE parsing is defensive: malformed JSON in the column yields an empty list rather than raising on a catalogue page. ION-side findings: 0C / 0H. 0.81.1 — **Closes an information asymmetry between the AI and the analyst; no new endpoint, permission, schema or data flow.** Two advisory annotations the alerts API already attached — System Quirks since v0.57.0 and KEV status, new here — now reach the person triaging. The KEV catalogue ships inside the image and is read locally; nothing fetches cisa.gov at runtime, which an existing test enforces. **Security-relevant as a positive**: `kev_prompt_block` has given Bob known-exploited status as ground truth since v0.79.1, so the model was better informed than the human about the same alert. That is the wrong way round for a decision a human owns, and it is now corrected. No alert becomes visible that was not, and no field is exposed that the caller could not already read under `alert:read` — both annotations are derived from data already in the response and from a catalogue of public information. **A KEV miss is never recorded as safety**: only hits are attached, because a dated snapshot cannot know about a CVE published after it was cut and an explicit negative invites reading absence as evidence. Both annotations are strictly additive: neither suppresses, hides, down-ranks nor reorders an alert, consistent with the System Quirks no-suppression posture, and quirks are rendered in a non-severity colour so advisory context cannot be misread as a severity claim. Availability: one indexed `IN` query per page rather than one per alert, and both annotators are best-effort — a lookup failure leaves the alerts view intact rather than emptying it. ION-side findings: 0C / 0H. 0.81.0 — **A presentation-layer rebuild plus one widened read; no new endpoint, permission or schema.** The alert list becomes a keyboard-driven triage queue. No route, gate or record changes: the queue renders what `/api/elasticsearch/alerts` and the existing batch-triage endpoint already returned to the same caller under `alert:read`. The batch endpoint returns four additional fields for alerts the caller can already read — assignee username, Bob's suggested verdict and confidence, and an observable count plus worst threat level — so **no alert becomes visible that was not**, and nothing is returned that the alert detail did not already expose. Observable values are summarised to a count and a level rather than shipped in full for up to 500 alerts, which reduces payload and keeps raw indicator values off the list surface. Threat level resolves through one indexed `IN` query per batch, and `case` and `assigned_to` are eager-loaded: both are read per row, so lazy-loading would issue one query per alert — the denial-of-service-by-design shape addressed at v0.79.4. **AI-safety relevant and unchanged in posture**: Bob's verdict is rendered in the AI accent colour, never a severity colour, always with its confidence, and never suppresses, hides or reorders an alert; the "Bob disagreed" view surfaces rows where the model and the severity conflict rather than acting on them. Filter values reach the DOM through `innerHTML` and are escaped, which is covered by test. No inline `style=` attribute is introduced — computed widths are DOM property writes, so `style-src-attr 'none'` is unaffected. ION-side findings: 0C / 0H. 0.80.3 — **A client-side correctness fix; no endpoint, permission, schema or data-flow change.** Two /pcap render functions dereferenced an undefined variable and threw before writing their table, so TLS-certificate and host-profile panels stayed blank whenever a capture actually contained that data. Security-relevant only as loss of analyst visibility: the certificate panel is where a self-signed or known-malicious C2 certificate would be seen, so the failure quietly withheld evidence rather than exposing any. Nothing was rendered incorrectly and no unauthorised field became readable — the panels rendered nothing at all. The removed highlight referenced a field absent from both record types, so no detection logic is lost. Arkime auto-case creation was exercised against a live database as part of this release and behaves as designed: the both-fields gate holds, cases are attributed to the AI user rather than a human, and the already-triaged guard makes repeat passes idempotent. ION-side findings: 0C / 0H. 0.80.2 — **Source-comment hygiene plus one client-side toggle fix; no endpoint, permission, schema or data-flow change.** 511 leading `# vX.Y.Z:` change-history annotations are removed across 113 files, with the record kept in the CHANGELOG instead; every file was verified byte-identical outside its comments, so this cannot alter behaviour, and comments that state a security-relevant constraint (the index-predicate coupling, the absence of 'unsafe-eval', pre-migration deployment boundaries) are deliberately retained. The single functional change is on /alerts: the analytics drawer decided its open/closed state from the inline `style.display`, which is blind to the class that actually hides it, so the first click was always a no-op. It now reads `getComputedStyle`. No authorisation, no data, no request is involved. A sweep of all 22 class-hidden elements against every `.style.display` read in the estate confirms this was the only true instance. ION-side findings: 0C / 0H. 0.80.0 — **A documentation correction with a code sweep behind it; no new endpoint, permission, schema or data flow.** This assessment and `server.py` both carried the v0.31.21 claim that inline `style=` attributes had been fully retired and P11 closed. They had not: 22 survived across 11 templates and scripts. The CSP was never weakened and was refusing them correctly — the defect was markup that depended on a declaration the policy forbids, so the failure mode was cosmetic-but-silent (unstyled elements, no error, no log) rather than an exposure. All 22 are removed: static declarations become CSS classes, computed ones move to `data-ion-style` applied via `el.style.setProperty`, which is a DOM property write and needs no `unsafe-inline`. A regression test now fails the build on any inline `style=` outside the email and PDF templates, which are exempt because they are not served under this policy. Net effect on posture: the directive is unchanged and now actually holds across the estate, and the false closure claim is corrected in both places it appeared. ION-side findings: 0C / 0H. 0.79.5 — **Client-side correctness and two CSP regressions closed; no new endpoint, permission, schema or data flow.** The shared alert-detail component was throwing ReferenceError on /cases for fourteen helpers declared only in alerts.html, leaving sections permanently unresolved; it now owns them, probing the host's implementation with a guarded bare reference rather than a string `typeof` — deliberately, because the CSP grants no 'unsafe-eval' and an eval-based probe would fail closed only in production. Two genuine CSP violations are fixed rather than excepted: the case-rail severity stripe and an inline `padding-top` that the v0.79.0 /documents rebuild reintroduced into a surface cleaned at v0.31.21 — both were being refused by `style-src-attr 'none'`, so the policy was working; the markup was not. A regression test now fails the build on any inline `style=` in either template, closing the gap that let one back in. Load-wise this is a REDUCTION: opening a case alert drops from nine authenticated reads to two, with the remainder deferred until a tab is opened, which removes a per-click amplification against Elasticsearch. Sections whose loader the host cannot provide are no longer rendered, so no control is shown that cannot act. Comments are read from the existing `GET /triage` under its existing gate and posted to the existing `alert:triage` endpoint — no new surface. ION-side findings: 0C / 0H. 0.79.4 — **Availability hardening on an existing read path; no new endpoint, permission, schema, data flow or field.** A production APM finding (~50% of response time in repeated `alert_triage` selects) is traced to the lab-fixture lookup that runs on every return path of the alerts-list endpoint: a prefix `LIKE` that cannot use the existing btree under a non-C collation, so it sequentially scanned a table that grows one row per triaged alert. Resolved with a partial index covering only the fixture rows — a query-plan change, not an access change: the same rows are returned to the same callers under the same `alert:read` gate. Security-relevant only in the denial-of-service-by-design sense, and favourably: it removes an unbounded per-request cost that grew with retained triage history, which is the shape that turns normal analyst load into self-inflicted exhaustion on a long-lived deployment. The migration is idempotent and non-fatal on failure (degrades to the previous scan, never to a wrong result), and no user-supplied value reaches the DDL. ION-side findings: 0C / 0H. 0.79.3 — **Strictly reduces what the client reads; no new surface.** The blank Host/User cells on a case are now filled from the observables ION already extracted at triage time and stores on `AlertTriage.observables`, which `/api/cases/{id}` was already returning to the same caller under the same `case:read` gate. That REMOVES a dependency rather than adding one: the values no longer require a second read of the alert's raw document, so they render without touching Elasticsearch at all, and no field is displayed that the response did not already contain. Only empty cells are filled, so a server-supplied value can never be overwritten by client-side inference, and malformed rows in the free-form observables JSON are skipped rather than raising. ION-side findings: 0C / 0H. Prior per-version net-new-surface notes: see CHANGELOG + git history.
**Pending / unreleased (branch `feat/aegis-service-account`, not yet merged or version-tagged):** adds a fourth service account, `aegis`, for the companion AEGIS threat-modelling app to call ION's API. The account is scoped to a new `aegis` RBAC role carrying exactly four permissions — `alert:read`, `de:read`, `de:propose`, `case:read` — deliberately excluding `de:verify` and `de:approve` (least privilege, SECURE_BY_DESIGN P6: AEGIS may read and draft detection proposals but never verify system quirks or approve Bob-tuning changes, both reserved for human reviewers). Authentication introduces **no new primitive**: `mint_service_account_token()` creates a `UserSession` row exactly as the OIDC callback does — a `secrets.token_urlsafe(32)` value whose SHA-256 hash (not the plaintext) is what's persisted, same as every other session in the system — and the resulting bearer token authenticates via the existing `validate_session()` path (`Authorization: Bearer`). The account is flagged `is_service_account=True`, which the pre-existing `login()` guard uses to hard-block interactive/password login before any password verification is attempted (same sentinel-hash mechanism as the `bob` service account). Seeding (`seed_aegis_user()`, wired into both `ion seed-aegis` and server startup's `_seed_auth()`) is idempotent — safe to re-run, tops up missing flags/role, never errors — and token minting is a separate, explicit, one-time CLI step (`ion seed-aegis --ttl-days`) so the credential does not silently rotate on every server restart. ION-side findings: 0C / 0H.
...
| Severity | Open findings (v0.81.2) |
|----------|:-----------------------:|
| Critical | 0 |
| High | 0 |
| Medium | 0 |
