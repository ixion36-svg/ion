...
**Assessment Date:** 2026-08-05 (current: v0.77.1). Per-version deltas and history: see CHANGELOG + git history.
**Application Version:** 0.77.1 — **UI ordering only; no surface change.** The case-detail alert rail is sorted by severity so the highest-severity alert is the one presented first. No endpoint, permission, schema or auth dependency touched; no data becomes newly visible — the same alerts were always listed, in a different order. Marginally positive for triage: a critical alert in a multi-alert case is no longer below the fold behind a low-severity one. ION-side findings: 0C / 0H. 0.77.0 — **UI consolidation; no new surface, and the CSP posture is unchanged.** One shared alert-detail component now renders an alert for both `/alerts` and `/cases`, replacing a partial second implementation and three copy-pasted helpers. **No endpoint, permission, schema or auth dependency was added, widened or removed** — `/cases` calls the same `/api/elasticsearch/alerts/*` reads it always could, under the same `case:read` page gate, and the component is served as a **static script** so it carries no inline handlers: every control uses `data-click-action` through the existing delegation helper, needing no `script-src-attr` relaxation. Security-relevant in one respect only, and positively: an analyst working a case now sees the alert's full evidence (raw data, parsed fields, sequence, related alerts) without leaving the case, where before the narrow panel hid it behind eight interactions — a completeness gain for triage, not an access change. ION-side findings: 0C / 0H. 0.76.0 — **Availability fix on a reported production failure; no new surface.** `/api/integrations/status` could return 500 for the whole request from two causes: a `**` unpack over the `details` JSON column when it held a non-mapping value, and the absence of per-connector isolation, so one faulting connector took down the status of all six. Both are **denial-of-visibility** defects rather than access-control ones — the page that reports integration health became unavailable exactly when an integration was unhealthy, which is when an operator most needs it. Each connector is now isolated and a fault renders as one named error card. The new user-facing error string is routed through `safe_error()` (test asserts no file paths reach the browser) and the full trace goes only to the server log. No route, permission, schema or auth dependency added or altered. ION-side findings: 0C / 0H. 0.75.0 — **Page relocation with no permission delta, plus an error-disclosure change that is deliberately conservative.** `/translator` retired into `/tools` as a tab (302 kept); both sides were already `alert:read`, so nothing was widened, narrowed or removed, and no endpoint changed. The `/integrations` page now surfaces the HTTP status and the server's `detail` on a failed load instead of one opaque sentence. That is an intentional, bounded increase in what an authenticated user is told about a failure: it exposes only what the API already returned to that same session, `detail` on ION's error paths is already routed through `safe_error()` (no file paths, library internals or stack frames), and the two messages that are hand-written rather than server-supplied — 401 and 403 — state the condition without naming which permission is missing. Net: an authenticated caller learns nothing it could not learn by reading the same response in devtools, while a failure becomes reportable instead of anonymous. Also fixes a latent `window.event` dependency in the tools tab switcher. ION-side findings: 0C / 0H. 0.74.0 — **Page merge; the one permission delta is a widening of the VIEW, not the DATA.** `/topology` is retired into `/integrations` as a tab (302 redirect kept). The retired page gate was `security:read` and `/integrations` is `alert:read`, so more roles can now reach the diagram. No data becomes newly reachable: every endpoint it reads already sits at `get_current_user` or `template:read`, and `require_integration_access` explicitly admits `alert:read` on the two endpoints both pages already shared — the page gate was fronting APIs that were never gated to match it, which is itself the kind of mismatch this consolidation exists to remove. No endpoint, permission, schema or auth dependency was added, widened or removed. ION-side findings: 0C / 0H. 0.73.0 — **UI-only fix; no new surface, and the CSP posture is unchanged.** Restores styling the v0.31.21 inline-style migration broke (125 dead CSS rules across 29 templates). The dynamic declarations move back to the element as a `data-ion-style` attribute and are applied by a new static script through `el.style.setProperty` — a DOM property write, **not** an inline `style=` attribute, so this needs no `style-src 'unsafe-inline'` relaxation and the existing CSP is untouched. `setProperty` validates its input, so a malformed value is ignored rather than injected; the applier never touches `innerHTML` or `cssText`. No route, permission, schema or data change. ION-side findings: 0C / 0H. 0.72.0 — **Legacy tuning queue retired into the DE module (route audit phase 8): a governance fix.** Bob filed tuning proposals unattended into a queue whose only review UI was unreachable, so autonomous output accumulated with no human ever seeing it. Those proposals now land in the DE module's `DetectionProposal` queue, which carries a decision record, provenance (`source=bob` vs human, plus the originating alert and investigation) and outcome measurement — the reviewer can now tell an inference drawn from a single alert from an analyst's campaign-driven draft. Bob still writes only a DRAFT; it never auto-applies. **Permission change, ordered deliberately:** `tuning:read`/`tuning:review` were removed only *after* their sole remaining enforcement point (Detection Health's file-proposal action) was re-gated to `de:propose`, which the same reviewer tier already holds — removing a seeded permission while it is still enforced would lock out every user including admin. No permission was widened. A startup migration carries still-pending legacy rows across idempotently so none are stranded. ION-side findings: 0C / 0H. 0.71.0 — **Page merges (route audit phase 7): permission boundaries deliberately preserved.** Four duplicated pages retired into the surfaces they duplicated (302 redirects kept; no API removed). Two page pairs — `bug-reports`/`change-requests` and `de-metrics`/`detection-health` — were **NOT** collapsed as originally planned, because each pair carries different permissions (`require_page_auth` vs `system:settings`; `security:read` vs `de:read`) and a single page would have required broadening one of them. They share a navigation tab strip instead, with each tab rendered only to users holding the corresponding permission — verified across every permission combination, including that a `de:read`-only user is never shown that Detection Health exists. **No permission was granted, widened, or removed; no schema change.** Corrects two reporting-accuracy defects: `/ai-scorecard`'s headline agreement figure was computed from an over-counted denominator, and four `/analytics` bar charts had rendered at zero width since v0.31.21 (dead migrated CSS). ION-side findings: 0C / 0H. 0.70.0 — **Page cleanup (route audit phase 6): one credential surface removed, no new surface.** The GitLab page carried its own configuration modal that wrote the same `.ion/config.json` fields as `/settings#gitlab` **and handled the raw personal access token in a form that `/settings` masks on read** — that second credential-handling surface is now gone, leaving a single administered path. Three page routes renamed to remove near-identical URLs (`/briefings` vs `/briefing`; `/investigate` vs `/investigations`), each retaining a 302 so bookmarks and any external links keep working; API namespaces untouched. A client-side handover builder on `/daily-work` was removed in favour of the server-side report. No permission, schema or auth dependency changed. ION-side findings: 0C / 0H. 0.69.0 — **Metric consolidation (route audit phase 5): internal refactor, no surface change.** Three duplicated computations collapsed to shared helpers (AIFeedback dedupe, case metrics, Bob-eval scoring). **No route, permission, schema or auth dependency was added or altered.** Integrity-relevant: `/ai-scorecard` had drifted from the other Bob-quality readers by counting circuit-breaker abstentions in its denominator, so an operator comparing surfaces saw inconsistent agreement rates for identical data — now unified; and `/bob-eval` reported Precision/Recall/F1 that were mathematically forced to be identical, i.e. a measurement surface that could not detect the regression it exists to detect. Both are accuracy-of-reporting fixes, not access-control changes. ION-side findings: 0C / 0H. Prior per-version net-new-surface notes: see CHANGELOG + git history.
...
| Severity | Open findings (v0.77.1) |
|----------|:-----------------------:|
| Critical | 0 |
| High | 0 |
| Medium | 0 |
