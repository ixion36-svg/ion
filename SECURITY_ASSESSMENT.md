...
**Open at v0.99.1 — 1 Low (accepted, not yet remediated):** sign-in response time differs measurably between an existing and an absent account, because a real account performs a password hash comparison and an absent one returns early (~200ms median, measured). v0.99.1 stopped *publishing* that measurement in a response header, which had made it trivially readable, but the difference itself remains observable over the network. Remediation is a constant-time sign-in path (perform a dummy hash comparison when the account does not exist); it is deferred because it changes the authentication path and warrants its own change.

| Severity | Open findings (v0.99.1) |
|----------|:-----------------------:|
| Critical | 0 |
| High | 0 |
| Medium | 0 |
