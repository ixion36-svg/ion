...
**Assessment Date:** 2026-08-24 (current: v0.83.0). Per-version deltas and history: see CHANGELOG + git history.
**Application Version:** 0.83.0 — **Net-new surface: none.** The /about briefing decks were re-authored as in-repo SVGs (content refresh; same fixed-lookup routing, same CSP posture) and the deck tab strip now renders from the route's dict — no new route, parameter, permission, or dependency. Six stale PDF/PPTX binaries and 38 PNGs were REMOVED from the shipped image. Prior version: 0.82.0 — **Net-new surface: one service account, one scoped role, one token-minting CLI step, and two startup repairs.** Adds a fourth service account, `metis`, for the companion METIS threat-modelling app to call ION's API. The account is scoped to a new `metis` RBAC role carrying exactly four permissions — `alert:read`, `de:read`, `de:propose`, `case:read` — deliberately excluding `de:verify` and `de:approve` (least privilege, SECURE_BY_DESIGN P6: METIS may read and draft detection proposals but never verify system quirks or approve Bob-tuning changes, both reserved for human reviewers). Authentication introduces **no new primitive**: `mint_service_account_token()` creates a `UserSession` row exactly as the OIDC callback does — a `secrets.token_urlsafe(32)` value whose SHA-256 hash (not the plaintext) is what's persisted — and the bearer token authenticates via the existing `validate_session()` path. The account is flagged `is_service_account=True`, which the pre-existing `login()` guard uses to hard-block interactive login before any password verification is attempted. Seeding is idempotent and wired into both `ion seed-metis` and startup's `_seed_auth()`; token minting is a separate explicit CLI step so the credential does not silently rotate on every restart. **Three findings from the review of that path are closed here, not deferred**: the mint had no audit entry unlike `login()`/OIDC and now writes one; re-minting was purely additive, so a leaked token survived the documented recovery step, and now rotates via `delete_all_for_user()` before minting; and `validate_session()`'s sliding window reset `expires_at` to now+24h whenever less than half the lifetime remained, so an actively-polling service account never reached `--ttl-days` and held a perpetual rolling session — service-account sessions now expire exactly at their minted expiry, with interactive human sessions unchanged. **Availability-relevant**: one legacy-tuning carry-over row written with enum values rather than names raised `LookupError` during hydration and failed `GET /api/de/proposals` for every caller — a whole endpoint denied by one malformed row, and the same endpoint METIS health-probes, so the failure read as "ION is down" from the other product. The INSERT is fixed and `repair_detection_proposal_enums` rewrites existing rows at startup, idempotently, leaving any value it cannot place alone rather than guessing at provenance. The AEGIS→METIS rename renames the service user and role **in place, preserving row ids**, so an already-issued bearer token is not revoked; it skips either row when a `metis` one already exists, because renaming onto a unique constraint would take startup down. ION-side findings: 0C / 0H. Prior per-version net-new-surface notes: see CHANGELOG + git history.
...
| Severity | Open findings (v0.83.0) |
|----------|:-----------------------:|
| Critical | 0 |
| High | 0 |
| Medium | 0 |
