...
**Assessment Date:** 2026-08-10 (current: v0.79.1). Per-version deltas and history: see CHANGELOG + git history.
**Application Version:** 0.79.1 — **New bundled data and one new administrative write path; no new egress and no new reachable data.** The CISA KEV catalog ships INSIDE the image and is seeded locally: **nothing in the application fetches cisa.gov at runtime**, which is a deliberate constraint for air-gapped deployment and is enforced by a test that fails if any application module pairs a network call with that host. The only fetching code is `scripts/refresh_kev_snapshot.py`, a developer tool run on a connected machine at release time; it validates the payload (version present, 500+ entries, records shaped like KEV) before overwriting the bundle, so a truncated download or an error page cannot silently replace good security data. The **operator import** is the one new write: gated on `system:settings`, capped at 12 MB, parsed and validated in full BEFORE any row is written (so a malformed upload cannot leave a half-replaced catalog), and audited with the resulting catalog version and row deltas. It replaces reference data for every user of the deployment, which is why it sits at the administrative tier rather than with analyst permissions. Read surfaces are `alert:read` — KEV membership is triage context, and the catalog is public information, so nothing sensitive is exposed by it. **AI-safety relevant, and positively**: KEV status is resolved in code and injected into Bob's prompts as ground truth rather than left to model recall, and a MISS is deliberately worded as absence-from-a-dated-snapshot — never as evidence the CVE is unexploited — because the failure mode that matters is a model reading a stale absence as safety for a CVE published after the snapshot. KEV listing is stated as advisory input, not an automatic verdict or severity, consistent with the System Quirks no-suppression posture. ION-side findings: 0C / 0H. 0.79.0 — **One permission model genuinely changes, and it narrows rather than widens; everything else is placement.** Document deletion moves from a blunt role permission to **ownership**: the endpoint gate becomes `document:read` (which every human role already holds) and the real rule — your own work, or a `document:delete` holder — is enforced **inside the endpoint, between loading the row and deleting it**, per the v0.20.1 TOCTOU lesson. Net effect on reach: the seven roles that could delete nothing can now delete **only what they created**, and the ~600 documents predating the new `created_by_id` column have no owner and stay behind `document:delete` — `None == None` would have made every legacy document everyone's to delete, so the guard requires a real owner id. Creates and deletes are now **audited** with the actor and, on delete, which authority was used, so an administrator removing another person's document is a distinguishable event from an author tidying their own. The new **GRC** menu gathers five surfaces that already existed (Compliance, Maturity, Security posture, Service desk, Audit logs) under one heading; each keeps its own distinct gate (`alert:read`, `security:read`, `system:settings`, `system:audit_view`), each item renders only to a holder, the group hides entirely for a user with none of them, and **the routes enforce regardless of what the menu shows** — a findability change, not an access change. `/documents` stops filtering out the Knowledge Base: those ~600 articles were always readable under the same `document:read` gate and were already searchable from the SOC workspace, so **no document becomes reachable that was not reachable before** — they simply become browsable. KB search now also matches article body text, which is a completeness gain within an unchanged permission boundary, and the collection filter validates its client-supplied id against the KB subtree so it cannot be used to read collections elsewhere in the library. ION-side findings: 0C / 0H. 0.78.0 — **Behaviour change in ION's autonomous AI, plus a new outbound write; no new route, permission or schema.** Bob's automatic run moves from a per-alert background sweep to **one cluster investigation per case at case creation**, and posts the result as a case comment. Three things make this a governance change worth stating rather than a refactor. **(1) Bob writes to the case record again.** v0.23.1 removed the unconditional auto-comment because one Bob note per alert buried analyst notes; the comment returns bounded to one per case, attributed to the Bob user, and **advisory only** — Bob still never sets a closure verdict or closes a case, and the confidence circuit-breaker on `suggested_verdict` is untouched. **(2) Adversary-controlled content is decoded and fed back into the prompt.** ION now base64-decodes PowerShell `-EncodedCommand` payloads deterministically and hands the model the plaintext as ground truth. The decoded text is attacker-controlled by definition, and it passes through the **same `_sanitize_alert_value` injection defence** as every other alert value on both the per-alert and cluster prompt paths — it is presented as data, never as instruction. Net posture is **positive**: a live test had the model invent a benign command for a real `IEX` download cradle, so removing the decode from the model's hands removes a class of hallucinated exoneration, and C2 indicators hidden inside base64 now become recorded observables instead of staying invisible. **(3) The case comment is mirrored to the linked Kibana case** — an outbound write to an already-configured, already-authenticated integration, over the same `sync_note_to_kibana` helper and credentials every other note path uses; best-effort and non-fatal, with no new endpoint or secret. Content-wise it carries Bob's verdict, summary and ATT&CK tags — the same analysis already visible in ION to anyone holding `case:read`. Also floors `setuptools>=83.0.0` (PYSEC-2026-3447), closing a pip-audit finding on a build dependency. ION-side findings: 0C / 0H. Prior per-version net-new-surface notes: see CHANGELOG + git history.
...
| Severity | Open findings (v0.79.1) |
|----------|:-----------------------:|
| Critical | 0 |
| High | 0 |
| Medium | 0 |
