...
from docforge.models.user import User, Role
...
class OIDCUserSync:
    """Synchronizes Keycloak users to DocForge database."""
    ...
    def sync_user(self, token_data: OIDCTokenData) -> User:
        """Find or create a DocForge user from OIDC token data.
        ...
        if user is None:
            # User doesn't exist in DocForge
    ...
    def _create_user(self, token_data: OIDCTokenData) -> User:
        """Create a new DocForge user from OIDC token data."""
    ...
    def _sync_roles(self, user: User, keycloak_roles: List[str]) -> None:
        """Synchronize user roles from Keycloak to DocForge.

        Uses direct name matching first, then falls back to configured mapping.
        """
        docforge_roles = self.map_roles(keycloak_roles)

        if docforge_roles:
            self.user_repo.set_roles(user, docforge_roles)
            logger.debug(
                f"Synced roles for {user.username}: {[r.name for r in docforge_roles]}"
            )

    def map_roles(self, keycloak_roles: List[str]) -> List[Role]:
        """Map Keycloak roles to DocForge roles.
