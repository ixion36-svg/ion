...
from ixion.core.config import get_dfir_iris_config

logger = logging.getLogger(__name__)

# Map IXION observable type to DFIR-IRIS ioc_type_id (integer)
IOC_TYPE_MAP = {
    "ip": 77,           # ip-dst
    "ip-src": 79,       # ip-src
    "domain": 20,       # domain
...
# Map IXION severity to DFIR-IRIS severity_id
SEVERITY_MAP = {
    "low": 2,
    "medium": 3,
    "high": 4,
...
class DFIRIRISService:
    ...
    async def _get_or_create_notes_directory(self, case_id: int) -> int:
        """Get or create an IXION notes directory for a case."""
        try:
            ...
            for d in dirs:
                if d.get("name") == "IXION Escalation Notes":
                    return d["id"]
            if dirs:
                return dirs[0]["id"]
        except Exception:
        ...
        try:
            result = await self._request(
                ...
                json={"name": "IXION Escalation Notes", "cid": case_id},
            )
    ...
    def map_ioc_type(self, observable_type: str) -> int:
        """Map an IXION observable type to a DFIR-IRIS IOC type ID."""
