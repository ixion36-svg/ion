...
from ixion.models.user import User, Role
...
class OIDCUserSync:
    """Synchronizes Keycloak users to IXION database."""
    ...
    def sync_user(self, token_data: OIDCTokenData) -> User:
        """Find or create a IXION user from OIDC token data.
    ...
    def _create_user(self, token_data: OIDCTokenData) -> User:
        """Create a new IXION user from OIDC token data."""
    ...
    def _sync_roles(self, user: User, keycloak_roles: List[str]) -> None:
        """Synchronize user roles from Keycloak to IXION.

        Uses direct name matching first, then falls back to configured mapping.
        """
        ixion_roles = self.map_roles(keycloak_roles)

        if ixion_roles:
            self.user_repo.set_roles(user, ixion_roles)
            logger.debug(
                f"Synced roles for {user.username}: {[r.name for r in ixion_roles]}"
            )

    def map_roles(self, keycloak_roles: List[str]) -> List[Role]:
        """Map Keycloak roles to IXION roles.
