...
from ixion.core.config import get_config, get_oidc_config
...
SESSION_COOKIE_NAME = "ixion_session"
...
def get_session_token(
    request: Request,
    ixion_session: Optional[str] = Cookie(default=None),
) -> Optional[str]:
    """Extract session token from cookie or Authorization header.

    Supports:
    - Cookie: ixion_session=<token>
    - Header: Authorization: Bearer <token>
    """
    # Try cookie first
    if ixion_session:
        return ixion_session
