"""Configuration management for IXION."""
...
class Config:
    ...
    db_path: Path = field(default_factory=lambda: Path.cwd() / ".ixion" / "ixion.db")
    ...
    def from_file(cls, path: Path) -> "Config":
        ...
        default_db_path = path.parent / "ixion.db"
...
def get_config() -> Config:
    ...
    2. Config file (.ixion/config.json)
    ...
    if _config is None:
        ...
        if os.environ.get("IXION_OIDC_REALM"):
            _config.oidc_realm = os.environ.get("IXION_OIDC_REALM", "")
        if os.environ.get("IXION_OIDC_CLIENT_ID"):
            _config.oidc_client_id = os.environ.get("IXION_OIDC_CLIENT_ID", "")
        if os.environ.get("IXION_OIDC_CLIENT_SECRET"):
            _config.oidc_client_secret = os.environ.get("IXION_OIDC_CLIENT_SECRET", "")
        ...
        if os.environ.get("IXION_GITLAB_TOKEN"):
            _config.gitlab_token = os.environ.get("IXION_GITLAB_TOKEN", "")
        if os.environ.get("IXION_GITLAB_PROJECT_ID"):
            _config.gitlab_project_id = os.environ.get("IXION_GITLAB_PROJECT_ID", "")
        ...
        if os.environ.get("IXION_OPENCTI_TOKEN"):
            _config.opencti_token = os.environ.get("IXION_OPENCTI_TOKEN", "")
        if os.environ.get("IXION_OPENCTI_VERIFY_SSL"):
            _config.opencti_verify_ssl = _get_env_bool("IXION_OPENCTI_VERIFY_SSL", True)
        ...
        if os.environ.get("IXION_ELASTICSEARCH_API_KEY"):
            _config.elasticsearch_api_key = os.environ.get("IXION_ELASTICSEARCH_API_KEY", "")
        if os.environ.get("IXION_ELASTICSEARCH_USERNAME"):
            _config.elasticsearch_username = os.environ.get("IXION_ELASTICSEARCH_USERNAME", "")
        if os.environ.get("IXION_ELASTICSEARCH_PASSWORD"):
            _config.elasticsearch_password = os.environ.get("IXION_ELASTICSEARCH_PASSWORD", "")
        if os.environ.get("IXION_ELASTICSEARCH_ALERT_INDEX"):
            _config.elasticsearch_alert_index = os.environ.get("IXION_ELASTICSEARCH_ALERT_INDEX", "")
        if os.environ.get("IXION_ELASTICSEARCH_CASE_INDEX"):
