...
def init(
    path: str = typer.Option(
        ".", "--path", "-p", help="Path to initialize ION in"
    ),
) -> None:
    ...
    config.to_file(ion_dir / "config.json")

    console.print(f"[green]OK[/green] Initialized ION in {ion_dir}")
    console.print(f"  Database: {db_path}")
    console.print(f"  Config: {ion_dir / 'config.json'}")
...
def status() -> None:
    ...
    console.print("[bold]ION Status[/bold]")
...
def upgrade() -> None:
    ...
    console.print("[bold]Upgrading ION database...[/bold]")
