...
def should_run_per_alert_sweep(config) -> bool:
    """Whether the standalone per-alert investigation sweep should run.

    Bob's automatic runs are meant to come from the case grouper (one cluster
    investigation per case at case creation). The old per-alert background sweep
    ran ``investigate_alert`` over every open alert and mostly timed out. So it is
    OFF whenever the case grouper is the active investigator, and only runs as a
    fallback when the grouper isn't investigating (and the loop is enabled).
    """
    if not getattr(config, "investigation_loop_enabled", True):
        return False
    grouper_investigating = (
        getattr(config, "case_grouper_enabled", True)
        and getattr(config, "case_grouper_auto_investigate", True)
    )
    return not grouper_investigating


def _render_bob_case_note(parsed: Dict[str, Any], cluster_size: int,
                          case_number: str = "") -> str:
    """Render Bob's case comment posted at case creation.

    Worded to distinguish a single-alert case from a multi-alert cluster
    ("analysed 1 alert" vs "analysed 3 alerts"). Kept short and scannable — the
    full analysis stays on the Investigation row / case-detail widget.
    """
    ...
    actions = (parsed.get("recommended_actions")
               or parsed.get("recommended_actions_structured") or [])
    mitre = parsed.get("mitre") or {}
    techs = mitre.get("techniques") if isinstance(mitre, dict) else (mitre or [])

    n = cluster_size or 1
    scope = "1 alert" if n == 1 else f"{n} alerts"
    lines: list[str] = [
        f"**🤖 Bob (AI analyst) — `{verdict}`** · severity `{severity}` · "
        f"confidence `{confidence}` · analysed **{scope}** in this case",
    ]
    if summary:
    ...
    if actions:
        ...
        for a in actions[:8]:
            text = a if isinstance(a, str) else (a.get("action") or str(a))
            lines.append(f"- {text}")
    if techs:
        lines.extend(["", "**ATT&CK:** " + ", ".join(str(t) for t in list(techs)[:12])])
    lines.extend(["", "_Auto-generated by Bob at case creation._"])
    return "\n".join(lines)
...
def _strip_jsonc(s: str) -> str:
    """Strip JSON-with-comments noise so ``json.loads`` accepts LLM output.

    Removes ``//`` line comments and ``/* */`` block comments **only outside
    string literals** (so real URLs like ``http://host/path`` survive) and drops
    trailing commas before ``}``/``]``. Foundation-Sec routinely emits both.
    """
    out: List[str] = []
    i, n = 0, len(s)
    in_str = False
...
def _parse_llm_json(content: str) -> Dict[str, Any]:
    ...
    # Strip //-comments / /* */ / trailing commas (string-aware) before parsing;
    # Foundation-Sec emits these routinely and they are invalid JSON.
    raw_block = _strip_jsonc(match.group(0))
    try:
...
class InvestigationService:
    ...
    async def _post_to_case(
        ...
        try:
            ...
            try:
                ...
                # Bob's case comment (per-case investigation only). One Note on
                # the case, worded for single- vs multi-alert. Best-effort: a
                # comment failure must not abort the other side-effects.
                if write_comment and (summary or verdict):
                    try:
                        from ion.services.ai_user import get_bob_user_id
                        bob_id = get_bob_user_id(db)
                        if bob_id is not None:
                            n = cluster_size
                            if not n:
                                n = len(case.source_alert_ids or []) or 1
                            body = _render_bob_case_note(
                                {"verdict": verdict, "severity": severity,
                                 "summary": summary, "recommended_actions": actions or [],
                                 "mitre": {"techniques": mitre_tags or []}},
                                cluster_size=n,
                            )
                            db.add(Note(
    ...
    def _build_user_prompt_body(
        ...
        # Deterministic deobfuscation: decode any PowerShell -EncodedCommand in the
        # alert *in code* and hand the model the plaintext as a fact. LLMs (incl.
        # Foundation-Sec) hallucinate base64 decodes — a live test had the model
        # invent a benign command for a real IEX download cradle — so we never ask
        # the model to decode; it only reasons over what we decoded.
        try:
            from ion.services.command_deobfuscate import deobfuscate_alert
            _decoded = deobfuscate_alert(alert_summary)
        except Exception:  # noqa: BLE001 - decode is best-effort, never break triage
            _decoded = []
        if _decoded:
            out.append("## Decoded command line (deterministic — DO NOT re-decode)")
            out.append(
                "These were base64-decoded by ION, not by you. Treat them as ground "
                "truth and reason over the plaintext; do not attempt your own decode."
            )
            for d in _decoded:
    ...
    async def investigate_alert(
        ...
        try:
            ...
            try:
                ...
                _blob = _alert_to_text_blob(alert)
                # Also extract IOCs from any deterministically-decoded
                # PowerShell -EncodedCommand payload — the C2 URL/IP/hash is
                # often only inside the base64, not in a raw alert field.
                _decoded = decoded_ioc_text(_blob)
                if _decoded:
                    _blob = _blob + "\n" + _decoded
                raw_iocs = extract_iocs(_blob)
            except Exception as exc:
