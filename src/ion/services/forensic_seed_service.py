"""Seed built-in Forensic Investigation playbooks on startup.

Creates 8 forensic playbooks with structured fields covering all investigation
...
def seed_forensic_playbooks() -> None:
    """Seed all built-in forensic investigation playbooks.

    Idempotent: skips playbooks that already exist (by name).
    """
    ...
    try:
        ...
        if not admin:
            logger.warning("Admin user not found, cannot seed forensic playbooks")
            return
        admin_id = admin.id

        # Get existing playbook names for idempotency
        existing = forensic_repo.list_playbooks()
        existing_names = {pb.name for pb in existing}

        if len(existing_names) >= len(PLAYBOOKS):
            # All playbooks likely already exist
            all_present = all(pb["name"] in existing_names for pb in PLAYBOOKS)
            if all_present:
                logger.info(
                    "Forensic playbooks already seeded (%d playbooks), skipping",
                    len(existing_names),
                )
                return
        ...
        for pb_data in PLAYBOOKS:
            ...
            forensic_repo.create_playbook(
                name=pb_data["name"],
                created_by_id=admin_id,
                description=pb_data.get("description"),
                investigation_type=pb_data.get("investigation_type"),
                is_active=True,
        ...
        session.commit()
        logger.info(
            "Forensic playbook seeding complete: %d created, %d already existed",
            created,
            len(PLAYBOOKS) - created,
        )
