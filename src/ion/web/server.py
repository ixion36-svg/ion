...
from ion.web.pinned_fields_api import router as pinned_fields_router
...
async def _startup_event():
    ...
    def _seed_auth():
        ...
        try:
            ...
            # Seed METIS — the threat-modelling integration service account.
            # Same idempotent shape as Bob; no token is minted here (that
            # would rotate METIS's credential on every restart) — operators
            # run `ion seed-metis` once to mint the bearer token out-of-band.
            auth_service.seed_metis_user()
            session.commit()
        finally:
            session.close()
