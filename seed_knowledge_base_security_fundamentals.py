...
concepts, common attack types, risk management basics, etc.

Organized into 4 collections under the existing 'Knowledge Base' parent:
  - Core Security Concepts        (~15 articles)
  - Network Models & Protocols    (~15 articles)
  - Authentication & Access       (~15 articles)
...
def upload_article(title, content, tags, collection_id):
    ...
    files = {"file": (f"{title}.md", md_bytes, "text/markdown")}
    form = {"name": title, "collection_id": str(collection_id), "tags": ",".join(tags)}
    r = SESSION.post(f"{BASE}/api/documents/upload", files=files, data=form)
    if r.status_code in (200, 201):
        doc_id = r.json().get("id", "?")
        print(f"    [{doc_id}] {title}")
        return doc_id
    else:
        print(f"    WARN upload failed ({r.status_code}): {title}")
        return None
...
def main():
    ...
    # Get Knowledge Base parent collection
    kb_parent = get_or_create_collection(
        "Knowledge Base",
        "Core knowledge base for SOC analysts",
    )
    if not kb_parent:
        print("[!] Could not find/create Knowledge Base parent collection")
        sys.exit(1)

    print(f"\n[*] Knowledge Base parent: id={kb_parent}")

    # Create collections
    collections = {
        "Core Security Concepts": get_or_create_collection(
            "Core Security Concepts",
            "Foundational security principles — CIA triad, defense in depth, threat landscape, frameworks",
            kb_parent,
        ),
        "Network Models & Protocols": get_or_create_collection(
            "Network Models & Protocols",
            "OSI model, TCP/IP, DNS, HTTP, email protocols, VPN technologies",
            kb_parent,
        ),
        "Authentication & Access": get_or_create_collection(
            "Authentication & Access",
            "Access control models, MFA, PKI, OAuth/SSO, Active Directory, password security",
            kb_parent,
        ),
        "Risk, Compliance & Operations": get_or_create_collection(
    ...
    # Upload articles
    article_groups = [
        ("Core Security Concepts", CORE_SECURITY),
        ("Network Models & Protocols", NETWORK_PROTOCOLS),
        ("Authentication & Access", AUTH_ACCESS),
        ("Risk, Compliance & Operations", RISK_COMPLIANCE),
    ]

    total = 0
    for group_name, articles in article_groups:
        col_id = collections[group_name]
        if not col_id:
            print(f"\n[!] Skipping {group_name} — collection not found")
            continue

        print(f"\n[+] {group_name} (collection id={col_id})")
        for article in articles:
            upload_article(article["title"], article["content"], article["tags"], col_id)
            total += 1
            time.sleep(0.05)

    print(f"\n[+] Done! Uploaded {total} security fundamentals articles.")
