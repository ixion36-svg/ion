...
The wizard's Step 2 form posts to the *same* /api/cyab/systems/{id}/answers
...
def authed_client(client):
    ...
    the intake answers API also depends on ``get_current_user`` directly (not
    via the page-permission factory). Add an override so POSTs to
    ``/api/cyab/systems/{id}/answers`` succeed without needing a real session
    token.
    """
...
def test_wizard_intake_answer_visible_from_per_system_page(authed_client):
    ...
    # Submit an intake answer directly to the answers endpoint — this is what
    ...
    r = authed_client.get(f"/api/cyab/systems/{sys_id}/answers")
